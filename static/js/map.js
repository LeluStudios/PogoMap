...
function loadNotifications(){ //@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@LENNY
...
function initSidebar() {
  ...
  $('#mobile-switch').prop('checked', Store.get('mobileNotification'));
...
function setupPokemonMarker(item, skipNotification, isBounceDisabled) {
  ...
  if (notifiedPokemon.indexOf(item.pokemon_id) > -1) {
    if (!skipNotification) {
     ...
     sendMobileNotification(item.pokemon_name);
    }
...
function getColorByDate(value) {
  //Changes the color from blue to red over 5 mins
...
function setupScannedMarker(item) {
 ...
 var offsetlng = 0.7 * 0.001280475;
 var offsetlat = 0.7 * 0.00045;
 var flightPlanCoordinates = [
    {lat: circleCenter.lat() + offsetlat*2, lng:  circleCenter.lng()},
    {lat: circleCenter.lat() + offsetlat, lng: circleCenter.lng() -offsetlng},
    {lat: circleCenter.lat() - offsetlat, lng: circleCenter.lng() -offsetlng},
    {lat: circleCenter.lat() - offsetlat*2, lng: circleCenter.lng()},
    {lat: circleCenter.lat() - offsetlat, lng: circleCenter.lng() +offsetlng},
    {lat: circleCenter.lat() + offsetlat, lng: circleCenter.lng() +offsetlng}
...
function clearStaleMarkers() {
  ...
  $.each(map_data.scanned, function(key, value) {
    ...
    if (map_data.scanned[key]['last_modified'] < (new Date().getTime() - 5 * 60 * 1000)) {
  ...
  $.each(markers, function(key, value) {
    if(id!=key && typeof  markers[key].marker != 'undefined' && typeof markers[id].marker != 'undefined'){
      if(google.maps.geometry.poly.containsLocation(markers[key].marker.getCenter(), markers[id].marker)){
         ...
         }else if(markers[key]['last_modified']>markers[id]['last_modified']){
...
function changeSearchLocation(lat, lng) {
 // return $.post("next_loc?lat=" + lat + "&lon=" + lng, {});
  $.post("http://localhost:5001" + "/next_loc?lat=" + lat + "&lon=" + lng);
  $.post("http://localhost:5002" + "/next_loc?lat=" + lat + "&lon=" + lng);
  $.post("http://localhost:5003" + "/next_loc?lat=" + lat + "&lon=" + lng);
  $.post("http://localhost:5004" + "/next_loc?lat=" + lat + "&lon=" + lng);
  $.post("http://localhost:5005" + "/next_loc?lat=" + lat + "&lon=" + lng);
  $.post("http://localhost:5006" + "/next_loc?lat=" + lat + "&lon=" + lng);
  return $.post("http://localhost:5000" + "/next_loc?lat=" + lat + "&lon=" + lng);
}
